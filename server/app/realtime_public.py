...
def public_realtime_payload(*, now_ms: int | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_at_epoch_ms": int(time.time() * 1000) if now_ms is None else int(now_ms),
        "lotteries": lotteries,
    }
...
def install() -> None:
    ...
    def public_realtime() -> dict[str, Any]:
        return public_realtime_payload()
